...
- [[#usage][Usage]]
  ...
  - [[#inserting-properties][Inserting properties]]
...
;; Insert a property
(define-key obsidian-mode-map (kbd "C-c C-M-p") 'obsidian-insert-property)
...
(use-package obsidian
  ...
  :bind (:map obsidian-mode-map
              ...
              ;; Insert a property
              ("C-c C-M-p" . obsidian-insert-property)))
...
Use the command ~obsidian-insert-property~ to insert or update a property in the
YAML front matter of the current note. This command will prompt for a property
name (with completion from existing properties in the vault) and a value.

#+begin_src
  M-x obsidian-insert-property RET property-name RET value RET
#+end_src
...
The Hydra menu includes the shortcut ~p~ for ~obsidian-insert-property~.
...
Other arbitrary properties (keys) are also supported and can be managed via
~obsidian-insert-property~.
